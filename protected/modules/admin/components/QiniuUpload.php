...
class QiniuUpload extends CApplicationComponent {
    public $accessKey;
    public $secretKey;
    //空间名字
    public $bucket;
    ...
    public static function getThumb($thumb_url,$width,$height,$quality=75){
        $domain = \Phalcon\DI::getDefault()->get('qiniuConfig')->domain;
        $width = (int)$width;
        $height = (int)$height;
        if(strpos($thumb_url,$domain)===0 && $width>0 && $height>0){
            $thumb_url .= '?imageView2/1/w/'.$width.'/h/'.$height.'/q/'.$quality;
        }
        return $thumb_url;
    }
